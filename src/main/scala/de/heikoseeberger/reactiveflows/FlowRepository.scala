...
object FlowRepository extends ExtensionKey[FlowRepository] {
  ...
  case class FlowAdded(flowData: FlowData)
    extends AddFlowReponse
    with Event
...
class FlowRepository(system: ExtendedActorSystem) extends Extension {
  ...
  private def get: Replicator.Get = Replicator.Get(ReplicatorKey, Replicator.ReadQuorum(settings.readTimeout))
