...
trait Sharding {
  ...
  def tellEntry(name: String, message: Any)(implicit sender: ActorRef): Unit = shardRegion ! (name, message)
  ...
  private def shardRegion: ActorRef = ClusterSharding(system).shardRegion(typeName)
}
